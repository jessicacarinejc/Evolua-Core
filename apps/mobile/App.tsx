...
function TodayScreen() {
  return (
    <ScrollView contentContainerStyle={styles.content} showsVerticalScrollIndicator={false}>
      ...
      <Text style={styles.subtitle}>Seu plano de hoje foi organizado para manter constância e evolução.</Text>

      <View style={styles.heroCard}>
        <View style={styles.heroTopRow}>
          <View>
            <Text style={styles.heroEyebrow}>TREINO DE HOJE</Text>
            <Text style={styles.heroTitle}>Pernas e Glúteos</Text>
          </View>
          <View style={styles.recoveryBadge}>
            <Text style={styles.recoveryValue}>86%</Text>
        ...
        <View style={styles.heroStats}>
          <View>
            <Text style={styles.heroStatValue}>48 min</Text>
          ...
          <View>
            <Text style={styles.heroStatValue}>Moderado</Text>
            <Text style={styles.heroStatLabel}>intensidade</Text>
          </View>
        </View>
      ...
      <View style={styles.metricGrid}>
        <MetricCard label="Energia" value="1.420" detail="de 1.850 kcal" />
        <MetricCard label="Proteína" value="92 g" detail="de 130 g" />
        <MetricCard label="Água" value="1,4 L" detail="de 2,5 L" />
        <MetricCard label="Passos" value="6.240" detail="meta 8.000" />
      </View>

      <View style={styles.sectionCard}>
        <View style={styles.sectionCardHeader}>
          <View>
            <Text style={styles.cardEyebrow}>NUTRIÇÃO</Text>
            <Text style={styles.cardTitle}>Meta diária</Text>
          </View>
          <Text style={styles.cardAction}>Ver plano</Text>
        </View>

        <Text style={styles.progressText}>Calorias · 1.420 / 1.850 kcal</Text>
        <ProgressBar value={1420} total={1850} />
        <Text style={styles.progressText}>Proteína · 92 / 130 g</Text>
        <ProgressBar value={92} total={130} />
        <Text style={styles.progressText}>Água · 1,4 / 2,5 L</Text>
        <ProgressBar value={1.4} total={2.5} />
      </View>
      ...
      <View style={styles.sectionCard}>
        ...
        <Text style={styles.cardBody}>
          Sono, energia, dor muscular e tempo disponível ajudam o Evolua Core a adaptar o treino sem ignorar suas restrições.
        </Text>
        <TouchableOpacity style={styles.secondaryButton} activeOpacity={0.85}>
          <Text style={styles.secondaryButtonText}>Fazer check-in diário</Text>
        </TouchableOpacity>
      </View>
      ...
      <View style={styles.noticeCard}>
        ...
        <Text style={styles.noticeText}>
          Sugestões do aplicativo não substituem avaliação médica, nutricional ou de profissional de Educação Física.
        </Text>
      </View>
    </ScrollView>
  );
...
function PlaceholderScreen({ title }: { title: Tab }) {
  const descriptions: Record<Tab, string> = {
    Hoje: '',
    Treino: 'Treinos adaptativos, histórico de cargas, vídeos e recuperação.',
    Nutrição: 'Plano alimentar, diário, macros, restrições e substituições.',
  ...
  return (
    <View style={styles.placeholder}>
      ...
      <View style={styles.placeholderBadge}>
        <Text style={styles.placeholderBadgeText}>Módulo em construção</Text>
      </View>
    </View>
  );
}
...
export default function App() {
  ...
  const screen = useMemo(
    () => (activeTab === 'Hoje' ? <TodayScreen /> : <PlaceholderScreen title={activeTab} />),
    [activeTab],
  );
...
const styles = StyleSheet.create({
  ...
  heroTitle: { color: theme.colors.white, fontSize: 25, fontWeight: '800', marginTop: 6 },
  ...
  heroStatValue: { color: theme.colors.white, fontSize: 15, fontWeight: '800' },
  ...
  primaryButtonText: { color: theme.colors.navyDark, fontWeight: '900', fontSize: 16 },
  ...
  metricValue: { color: theme.colors.navy, fontSize: 22, fontWeight: '900', marginTop: 7 },
